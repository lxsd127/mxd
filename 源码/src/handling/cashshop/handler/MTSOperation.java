    ...
    public static void MTSOperation(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        if (op == 2) { //put up for sale
            ...
            if (invType != 1 && invType != 2) { //pet?
                c.getSession().write(MTSPacket.getMTSFailSell());
                doMTSPackets(cart, c);
                return;
            }
            ...
            if (ii.isCash(itemid) || quantity <= 0 || item == null || item.getQuantity() <= 0 || item.getItemId() != itemid || item.getUniqueId() > 0 || item.getQuantity() < quantity || price < MTSCSConstants.MIN_MTS || c.getPlayer().getMeso() < MTSCSConstants.MTS_MESO || cart.getNotYetSold().size() >= 10 || ii.isDropRestricted(itemid) || ii.isAccountShared(itemid) || item.getExpiration() > -1 || item.getFlag() > 0) {
                c.getSession().write(MTSPacket.getMTSFailSell());
                doMTSPackets(cart, c);
                return;
            }
            ...
            c.getPlayer().gainMeso(-MTSCSConstants.MTS_MESO, false);
            c.getSession().write(MTSPacket.getMTSConfirmSell());
	    ...
	    cart.changeCurrentView(MTSStorage.getInstance().getSearch(slea.readInt() > 0, slea.readMapleAsciiString(), cart.getType(), cart.getTab()));
            ...
            if (!MTSStorage.getInstance().removeFromBuyNow(slea.readInt(), c.getPlayer().getId(), true)) {
                c.getSession().write(MTSPacket.getMTSFailCancel());
            } else {
                c.getSession().write(MTSPacket.getMTSConfirmCancel());
            ...
            if (item != null && item.getQuantity() > 0 && MapleInventoryManipulator.checkSpace(c, item.getItemId(), item.getQuantity(), item.getOwner())) {
                ...
                if (pos >= 0) {
                    ...
                    c.getSession().write(MTSPacket.getMTSConfirmTransfer(GameConstants.getInventoryType(item_.getItemId()).getType(), pos)); //IF this is actually pos and pos
                    sendMTSPackets(cart, c, true);
                    return;
                } else {
                    c.getSession().write(MTSPacket.getMTSFailBuy());
                }
            } else {
                c.getSession().write(MTSPacket.getMTSFailBuy());
            }
            ...
            if (MTSStorage.getInstance().checkCart(id, c.getPlayer().getId()) && cart.addToCart(id)) {
                c.getSession().write(MTSPacket.addToCartMessage(false, false));
            } else {
                c.getSession().write(MTSPacket.addToCartMessage(true, false));
            }
        } else if (op == 10) { //delete from cart
            final int id = slea.readInt();
            if (cart.getCart().contains(id)) {
                cart.removeFromCart(id);
                c.getSession().write(MTSPacket.addToCartMessage(false, true));
            } else {
                c.getSession().write(MTSPacket.addToCartMessage(true, true));
            }
            ...
            if (mts != null && mts.getCharacterId() != c.getPlayer().getId()) {
                if (c.getPlayer().getCSPoints(1) > mts.getRealPrice()) {
                    if (MTSStorage.getInstance().removeFromBuyNow(mts.getId(), c.getPlayer().getId(), false)) {
                        ...
                        c.getSession().write(MTSPacket.getMTSConfirmBuy());
                        sendMTSPackets(cart, c, true);
                        return;
                    } else {
                        c.getSession().write(MTSPacket.getMTSFailBuy());
...
//          System.out.println("New MTS Op " + op + ", \n" + slea.toString());
	...
	final int a = MTSStorage.getInstance().getCart(c.getPlayer().getId()).getSetOwedNX();
        ...
        c.getSession().write(MTSPacket.getMTSWantedListingOver(0, 0));
        doMTSPackets(cart, c);
    }
    ...
    private static void sendMTSPackets(final MTSCart cart, final MapleClient c, final boolean changed) {
        ...
        c.getSession().write(MTSPacket.showMTSCash(c.getPlayer()));
