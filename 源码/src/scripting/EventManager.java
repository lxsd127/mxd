...
public class EventManager {
    ...
    public void setWorldEvent() {
        for (int i = 0; i < eventChannel.length; i++) {
            eventChannel[i] = 2; //2-13
        }
    }
    
    public final void getClock(MapleCharacter chr, int timeOnRide) {
        chr.getClient().getSession().write(MaplePacketCreator.getClock(timeOnRide));
    }
}
