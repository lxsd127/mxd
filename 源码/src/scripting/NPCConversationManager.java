...
public class NPCConversationManager extends AbstractPlayerInteraction {
  ...
  public void setboss(int slot) {
    ...
    public void sendYesNoS(String text, byte type, int idd) {
        ...
        c.getSession().write(MaplePacketCreator.getNPCTalk(id, (byte) 1, text, "", type, idd));
        lastMsg = 1;
    }
    ...
    public void sendSimpleS(String text, byte type, int idd) {
        ...
        c.getSession().write(MaplePacketCreator.getNPCTalk(id, (byte) 4, text, "", (byte) type, idd));
        lastMsg = 4;
    }
    ...
    public void increaseGuildCapacity(boolean trueMax) {
        if (c.getPlayer().getMeso() < 5000000 && !trueMax) {
            c.getSession().write(MaplePacketCreator.serverNotice(1, "你没有足够的金币来增加家族成员人数上限。"));
            return;
        }
        final int gid = c.getPlayer().getGuildId();
        if (gid <= 0) {
...
public static String SN取出售(int id) {
    ...
    public void gainBeans(int s) {
        ...
        c.getSession().write(MaplePacketCreator.updateBeans(this.c.getPlayer().getId(), s));
    }
    ...
    public void showlvl() {
        MapleGuild.showlvl(getClient(), getNpc());
    }
    public void 金币排行() {
        MapleGuild.meso(getClient(), getNpc());
    }

    public void 人气排行榜() {
        MapleGuild.人气排行(getClient(), getNpc());
    }
}