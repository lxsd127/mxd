...
public static boolean addById(MapleClient c, int itemId, short quantity, String owner, MaplePet pet, byte Flag) {
        return addById(c, itemId, quantity, owner, pet, 0, "通过控制台发放");
    }
    ...
    public static void equip(final MapleClient c, final short src, short dst) {
      ...
      //      c.getSession().write(MaplePacketCreator.enableActions());
       ...
       // }
        ...
        if ((dst == -122) && source.getItemId() == 1802100) {
            final Item top = chr.getInventory(MapleInventoryType.EQUIPPED).getItem((byte) -123);
            if (top == null) {
                dst = -123;
            } else {
                dst = -125;
