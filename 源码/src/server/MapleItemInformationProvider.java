...
//类型MapleData,方法getItemData,整数参数itemId
    protected final MapleData getItemData(final int itemId) {
        //声明了一个名称为ret的MapleData类型的变量
        MapleData ret = null;
        //字符串idStr。 = “0”+ itemId转换为字符
        final String idStr = "0" + String.valueOf(itemId);
        //从itemData中获取根目录，并存储在root变量中
        ...
        for (final MapleDataDirectoryEntry topDir : root.getSubdirectories()) {
            ...
            for (final MapleDataFileEntry iFile : topDir.getFiles()) {
                //检查文件名是否等于 idStr 的前四个字符加上 ".img"
                if (iFile.getName().equals(idStr.substring(0, 4) + ".img")) {
                    //如果是，则读取该文件的数据，并查找子路径名为 idStr的数据
                    ...
                    return ret;
                    //如果文件名不匹配，它则检查文件名是否等于 idStr 从第二个字符开始到最后的字符加上 ".img"
        ...
        //如果没有找到任何匹配的文件，方法返回 ret ，这时 ret 应该是 null
        return ret;
    }
                    ...
                    default: {
                        if (GameConstants.isChaosScroll(scrollId.getItemId())) {
                            ...
                            if (nEquip.getState() == 0) {
                                final int chanc = (scrollId.getItemId() == 5534000 || scrollId.getItemId() == 2049402 || scrollId.getItemId() == 2049406 ? 100 : (scrollId.getItemId() == 2049400 ? 90 : 70));
                                if (Randomizer.nextInt(100) > chanc) {
                                    return null; //destroyed, nib
                                }
    ...
    public int getLimitBreak(int itemId) {//PVP
