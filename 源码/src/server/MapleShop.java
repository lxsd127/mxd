...
public class MapleShop {
    ...
    public void sendShop(MapleClient c) {
        ...
        if (npc == null || npc.getName().equals("MISSINGNO")) {
            ...
            c.getPlayer().dropMessage(6, "您已建立与商店" + id + "的连接");
        }
        ...
        if (item != null && item.getPrice() > 0 && item.getReqItem() == 0) {
            ...
            if (price >= 0 && c.getPlayer().getMeso() >= price) {
                if (MapleInventoryManipulator.checkSpace(c, itemId, quantity, "")) {
                    if (getId() == 9120104) {
                        if (c.getPlayer().getBeans() < price) {
                            c.getPlayer().dropMessage(1, "您的豆豆数量不足,无法购买。");
                        } else {
                    ...
                    if (GameConstants.isPet(itemId)) {
                        ...
                        MapleInventoryManipulator.addById(c, itemId, quantity, "购买NPC商店物品: " + itemId + " " + id + ", " + npcId + "时间 " + FileoutputUtil.CurrentReadable_Date());
                    }
                } else {
            ...
            if (MapleInventoryManipulator.checkSpace(c, itemId, quantity, "")) {
                ...
                if (GameConstants.isPet(itemId)) {
                    ...
                    MapleInventoryManipulator.addById(c, itemId, quantity, "购买NPC商店物品: " + id + ", " + npcId + " 时间 " + FileoutputUtil.CurrentReadable_Date());
                }
            } else {
                c.getPlayer().dropMessage(1, "您的背包是满的，请整理下背包。");
            }
    ...
    public void sell(MapleClient c, MapleInventoryType type, byte slot, short quantity) {
        ...
        if (quantity <= iQuant && iQuant > 0) {
            ...
            double price;
            if (GameConstants.isThrowingStar(item.getItemId()) || GameConstants.isBullet(item.getItemId())) {
                price = ii.getPrice(item.getItemId());//price = ii.getWholePrice(item.getItemId()) / (double) ii.getSlotMax(item.getItemId());
            } else {
        ...
        try (Connection con = DBConPool.getInstance().getDataSource().getConnection()) {
            ...
            while (rs.next()) {
                ...
                if (GameConstants.isThrowingStar(rs.getInt("itemid")) || GameConstants.isBullet(rs.getInt("itemid"))) {
                    MapleShopItem starItem = new MapleShopItem((short) 1, rs.getInt("itemid"), rs.getInt("price"), rs.getInt("reqitem"), rs.getInt("reqitemq"), (byte) 0, 0/*, rs.getByte("rank"), rs.getInt("category")*/);
                    ret.addItem(starItem);
                    if (rechargeableItems.contains(starItem.getItemId())) {
                        recharges.remove(Integer.valueOf(starItem.getItemId()));
                    }
                } else {
                    ret.addItem(new MapleShopItem((short) 1000, rs.getInt("itemid"), rs.getInt("price"), rs.getInt("reqitem"), rs.getInt("reqitemq"), (byte) 0, 0/*rs.getByte("rank"), rs.getInt("category")*/));
                }
            }
