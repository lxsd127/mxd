                ...
                if (sponge.get() == null && hp > 0) {
                    switch (stats.getHPDisplayType()) {
                        case 0:
                            ...
                            break;
                        case 1:
                            map.broadcastMessage(from, MobPacket.damageFriendlyMob(this, damage, true), true/*false*/);
                            break;
                        case 2:
...
//              exp = ChannelServer.getInstance(map.getChannel()).getExpRate() / 5; // depend on your rate and /
        ...
        if (exp > 0) {
            ...
            int Friendship_Bonus_EXP = 0;
            if (attacker.getStat().equippedFriendshipRing) {
                Friendship_Bonus_EXP = (int) Math.ceil((exp / 100.0) * 15);
            }

            int Marriage_Bonus_EXP = 0;
            if (attacker.getStat().equippedMarriageRing) {
                Marriage_Bonus_EXP = (int) Math.ceil((exp / 100.0) * 25);
            }
            ...
            attacker.gainExpMonster(exp, true, highestDamage, pty, Class_Bonus_EXP, Equipment_Bonus_EXP, Premium_Bonus_EXP, stats.isPartyBonus(), stats.getPartyBonusRate(), Friendship_Bonus_EXP, Marriage_Bonus_EXP);
        }
 ...
 /*if (killer != null && stats.isBoss()) {
    ...
    private class PartyAttackerEntry implements AttackerEntry {
        ...
        public final void killedMob(final MapleMap map, final int baseExp, final boolean mostDamage, final int lastSkill) {
            ...
            byte Premium_Bonus_EXP, added_partyinc, Friendship_Bonus_EXP;
            ...
            for (final Entry<MapleCharacter, OnePartyAttacker> attacker : resolveAttackers().entrySet()) {
                ...
                for (final MaplePartyCharacter partychar : party.getMembers()) {
                    if (attacker.getKey().getLevel() - partychar.getLevel() <= 5 || stats.getLevel() - partychar.getLevel() <= 5) {
                        ...
                        if (pchr != null && pchr.isAlive()) {
                            ...
                            //if (pchr.getStat().equippedFriendshipRing && Friendship_Bonus_EXP == 0) {
                            //     Friendship_Bonus_EXP = 25;
                            //}
                ...
                for (final MapleCharacter expReceiver : expApplicable) {
                    ...
                    if (ServerConfig.EXP) {
                        iexp += (int) Math.ceil((innerBaseExp / expApplicable.size()));
                    } else {
