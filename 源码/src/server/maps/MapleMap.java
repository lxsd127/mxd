...
public final class MapleMap {
    ...
    private void dropFromMonster(final MapleCharacter chr, final MapleMonster mob, final boolean instanced) {
        ...
        int tempmeso = Math.min(30000, (int) (mesoDecrease * (mob.getStats().getExp()) * (1.0 + Math.random() * /*10*/ 5) / 10.0));
        if (tempmeso > 0) {
    ...
    public final void killMonster(final MapleMonster monster, final MapleCharacter chr, final boolean withDrops, final boolean second, byte animation, final int lastSkill) {
        ...
        if (mobid == 8810018 && mapid == 240060200) { // Horntail
            ...
            World.Broadcast.broadcastMessage(MaplePacketCreator.serverNotice(6, "經過無數次的挑戰，" + chr.getName() + "所帶領的隊伍終於擊破了混沌闇黑龍王的遠征隊！你們才是龍之林的真正英雄~"));
            charactersLock.readLock().lock();
            try {
                /*for (MapleCharacter c : characters) {
            ...
            World.Broadcast.broadcastMessage(MaplePacketCreator.serverNotice(6, "以永不熄滅的熱情所帶領的遠征隊打倒皮卡啾啊！" + chr.getName() + "你們是真正的勇者！真正的勝利者！"));
            charactersLock.readLock().lock();
            try {
                /*for (MapleCharacter c : characters) {
        ...
        spawnAndAddRangedMapObject(summon, new DelayedPacketCreation() {
            ...
            public void sendPackets(MapleClient c) {
                if (summon != null && c.getPlayer() != null && (!summon.isChangedMap() || summon.getOwnerId() == c.getPlayer().getId())) {
                    ...
                    if ((summon.getSkill() == 3111002 || summon.getSkill() == 3211002 || summon.getSkill() == 5211001 || summon.getSkill() == 5220002) && summon.getOwnerId() == c.getPlayer().getId()) {
                        broadcastMessage(MaplePacketCreator.spawnSummon(summon, true));
                    } else if (summon.getSkill() != 3111002 && summon.getSkill() != 3211002 && summon.getSkill() != 5211001 && summon.getSkill() != 5220002) {
                        broadcastMessage(MaplePacketCreator.spawnSummon(summon, true));
                    }
                }
            }
 ...
 /*
    ...
    public void respawn(final boolean force, final long now) {
        ...
        if (force) { //cpq quick hack
            final int numShouldSpawn = monsterSpawn.size() * ServerConfig.monsterSpawn - spawnedMonstersOnMap.get();
            ...
            final int numShouldSpawn = (GameConstants.isForceRespawn(mapid) ? monsterSpawn.size() * ServerConfig.monsterSpawn : maxRegularSpawn * ServerConfig.monsterSpawn) - spawnedMonstersOnMap.get();
            if (numShouldSpawn > 0) {
                int spawned = 0;
                ...
                for (Spawns spawnPoint : randomSpawn) {
                    ...
                    if (spawnPoint.shouldSpawn(lastSpawnTime) || GameConstants.isForceRespawn(mapid) || (monsterSpawn.size() * ServerConfig.monsterSpawn < 10 && maxRegularSpawn * ServerConfig.monsterSpawn > monsterSpawn.size() * ServerConfig.monsterSpawn && partyBonusRate > 0)) {
                        spawnPoint.spawnMonster(this);
                        spawned++;
                    }
                    if (spawned >= numShouldSpawn) {
