...
public class MapleQuest implements Serializable {
 ...
 public static void clearQuests() {
    ...
    public void start(MapleCharacter c, int npc) {
        if ((autoStart || checkNPCOnMap(c, npc)) && canStart(c, npc)) {
            ...
            if (!customend) {
                ...
                if (id == 6029) {
                    ...
                    //MapleQuest.getInstance(6031).forfeit(c);
                    //MapleQuest.getInstance(6032).forfeit(c);
                }
                if (id == 6033) {
                    MapleQuest.getInstance(6033).forceStart(c, 2111000, null);
                    //MapleQuest.getInstance(6031).forfeit(c);
                    //MapleQuest.getInstance(6032).forfeit(c);
                }
            } else {
        ...
        if (id == 6035) {
            MapleQuest.getInstance(6036).forceStart(c, 2111000, null);
    ...
    public void complete(MapleCharacter c, int npc, Integer selection) {
        if (c.getMap() != null && (autoPreComplete || checkNPCOnMap(c, npc)) && canComplete(c, npc)) {
            ...
            c.getClient().getSession().write(MaplePacketCreator.showSpecialEffect(0x9)); // Quest completion
            c.getMap().broadcastMessage(c, MaplePacketCreator.showSpecialEffect(c.getId(), 0x9), false);
        }
    }
