            ...
            try (PreparedStatement ps = con.prepareStatement("SELECT * FROM shopitems WHERE itemid = ? ORDER BY price")) {
                ...
                try (ResultSet rs = ps.executeQuery()) {
                    while (rs.next()) {
                        ...
                        if (ii.getPrice(itemId) * 2 > rs.getLong("price") && rs.getInt("price") != 0) {
                            System.out.println("更正物品价格, 物品ID: " + itemId + " 商店ID: " + rs.getInt("shopid") + " 原价格: " + rs.getLong("price") + " 改后价格:" + (long) ii.getPrice(itemId) * 2);
                            try (PreparedStatement pp = con.prepareStatement("UPDATE shopitems SET price = ? WHERE itemid = ? AND shopid = ?")) {
                                pp.setLong(1, (long) ii.getPrice(itemId) * 2);
                                pp.setInt(2, itemId);
